...
_STREET_PREFIXES = (
    r"(?:ул\.|улица|улице|пр\.|пр-т|проспект|проспекте|пер\.|переулок|переулке"
    r"|бульвар|бульваре|б-р|ш\.|шоссе|наб\.|набережная|набережной|пл\.|площадь|площади)"
)

# Числительные словами (для номеров домов: «дом пять», «дом двадцать три»)
# Важно: длинные слова (двадцать, тринадцать) ДО коротких (два, три), + \b для word boundary
_NUM_WORDS = (
    r"(?:двадцать|тридцать|пятьдесят|шестьдесят|семьдесят|восемьдесят|девяносто"
    ...
    r"|сорок|сто|первый|второй|третий|четвёртый|пятый)\b"
)

# Номер дома: цифрами или словами
_HOUSE_NUM = rf"(?:\d+[А-Яа-яA-Za-z]?|{_NUM_WORDS}(?:\s+{_NUM_WORDS})?)"
...
# Смешанный регистр: «андрей Храпков» (lowercase имя + Capitalized фамилия)
# Низкий приоритет, работает только в связке с PD-контекстом через detector.py
_FIO_MIXED = re.compile(
    rf"(?<![А-ЯЁа-яё])"
    rf"([а-яё]{{3,}})"
    rf"\s+"
    rf"([А-ЯЁ][а-яё]{{2,}})"
...
ALL_RULES.append(PDRule(
    category="fio",
    pattern=_FIO_MIXED,
    description="ФИО смешанный регистр (имя lowercase + Фамилия)",
    priority=35,
))
...
_FIO_ZVAT_CTX = re.compile(
    r"(?:меня\s+)?(?:зовут|звать|имя)"
    r"(?:\s+\S+)?"  # опциональное слово между («звать его», «имя моё»)
    r"[\s:]+"
    rf"({_CYR_WORD_ANY}\s+{_CYR_WORD_ANY}(?:\s+{_CYR_WORD_ANY})?)",
    re.IGNORECASE | re.UNICODE,
)
ALL_RULES.append(PDRule(
    category="fio",
    pattern=_FIO_ZVAT_CTX,
    description="ФИО после «зовут/звать/имя» (любой регистр)",
    priority=78,
    context_required=True,
    use_group=1,
))
...
# Адрес: Название + тип улицы (обратный порядок, напр. "Красный проспект, д. 50" или "невском проспекте")
_ADDRESS_STREET_REV = re.compile(
    rf"(?:(?:г\.|город)\s+[А-ЯЁа-яё\-]+[\s,]*?)?"
    rf"[А-ЯЁа-яё][а-яё]+(?:[\-][А-ЯЁа-яё][а-яё]+)?(?:\s+[а-яё]+)?\s+"
    rf"(?:проспекте|проспект|бульваре|бульвар|шоссе"
    rf"|набережной|набережная|площади|площадь"
    rf"|переулке|переулок|улице|улица)"
    rf"(?:,?\s*(?:д\.|дом)\s*{_HOUSE_NUM}"
    rf"(?:(?:,?\s*(?:корп?\.|корпус)\s*\d+)?"
    rf"(?:,?\s*(?:кв\.|квартира|оф\.|офис)\s*\d+)?)?)?" ,
    re.IGNORECASE | re.UNICODE,
...
# Адрес с предлогом + город (напр. "на невском проспекте дом пять в санкт-петербурге")
_CITY_NAMES = (
    r"(?:москв[аеыуой]|петербург[аеуом]?"
    r"|санкт-петербург[аеуом]?"
    r"|новосибирск[аеуом]?|екатеринбург[аеуом]?"
    r"|казан[иью]?|нижн[a-яё]{2,10}новгород[аеуом]?"
    r"|челябинск[аеуом]?|самар[аеуоый]?"
    r"|омск[аеуом]?|ростов[аеуом]?"
    r"|уф[аеуоый]?|красноярск[аеуом]?"
    r"|перм[иью]?|воронеж[аеуом]?"
    r"|волгоград[аеуом]?)"
)
_ADDRESS_STREET_REV_CTX = re.compile(
    rf"(?:^|(?<=\s))(?:на|по|в|во)\s+"
    rf"[А-ЯЁа-яё][а-яё]+(?:[\-][А-ЯЁа-яё][а-яё]+)?(?:\s+[а-яё]+)?\s+"
    rf"(?:проспекте|проспект|бульваре|бульвар|шоссе"
    rf"|набережной|набережная|площади|площадь"
    ...
    rf"(?:(?:,?\s*(?:корп?\.|корпус)\s*\d+)?"
    rf"(?:,?\s*(?:кв\.|квартира|оф\.|офис)\s*\d+)?)?)?"
    rf"(?:\s+в\s+{_CITY_NAMES})?",
    re.IGNORECASE | re.UNICODE,
)
ALL_RULES.append(PDRule(
    category="address",
    pattern=_ADDRESS_STREET_REV_CTX,
    description="Адрес (предлог + название + тип улицы + город)",
    priority=63,
))
...
# Обфусцированный телефон: цифры смешаны со словами-числительными
# Примеры: +7911000пять33, 8девять111234три
_DIGIT_WORD = r"(?:ноль|один|два|три|четыре|пять|шесть|семь|восемь|девять)"
_PHONE_MIX_UNIT = rf"(?:\d|{_DIGIT_WORD})"
_PHONE_OBFUSCATED = re.compile(
    rf"(?:\+7|8)"
    rf"(?:{_PHONE_MIX_UNIT}[\s\-]*){{7,}}",
    re.IGNORECASE | re.UNICODE,
)
ALL_RULES.append(PDRule(
    category="phone",
    pattern=_PHONE_OBFUSCATED,
    description="Телефон РФ обфусцированный (цифры + слова)",
    priority=75,
))
